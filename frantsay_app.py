...
if not st.session_state.identified:
    st.markdown("## Bienvenue sur FRANTSAY")
    st.markdown(
        '<p style="color:var(--muted);font-size:.85rem;margin-top:-.4rem">'
        "Indique ton e-mail et ton pseudo pour créer ton profil et retrouver "
        "ta progression à chaque visite. Aucun mot de passe n'est demandé."
        "</p>",
...
# Interactivité du clic sur la mascotte robot : l'ancien attribut inline
# onclick="...el.classList.add('robot-clicked')..." ne redémarrait PAS
# l'animation robotClickBounce en cas de clics rapprochés, car ré-ajouter une
# classe déjà présente sur l'élément n'a aucun effet pour le navigateur (pas
# de reflow forcé). On attache donc un vrai gestionnaire d'évènement en JS,
# qui force un reflow avant de réappliquer la classe afin que chaque clic,
# même rapproché, relance bien l'animation. Le binding est retenté tant que
# le SVG n'est pas encore monté dans le DOM (composant asynchrone).
ROBOT_CLICK_JS = """
<script>
(function() {
    var win = window.parent || window;
