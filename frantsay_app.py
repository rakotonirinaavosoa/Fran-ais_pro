...
.st-key-audio_bridge_slot, .st-key-progress_bridge_slot { position: absolute; width: 1px; height: 1px; overflow: hidden; opacity: 0; pointer-events: none; }
...
# =============================================================================
# 5bis. PROGRESSION PERSISTANTE (localStorage du navigateur)
# =============================================================================
# Streamlit efface st.session_state à chaque actualisation de page (nouvelle
# session serveur). Pour ne pas perdre les points/niveau à chaque F5, on les
# sauvegarde côté navigateur (localStorage) et on les relit au chargement, via
# le même principe de "pont" caché que l'enregistreur audio du module 03 :
# un composant JS invisible écrit/relit un champ texte caché, ce qui déclenche
# un rerun Streamlit quand une valeur sauvegardée doit être restaurée.
# Remarque : cette progression est liée au navigateur/appareil utilisé, pas à
# un compte — pas besoin de créer un profil pour en profiter. Un vrai système
# de connexion (avec base de données) pourra remplacer ce mécanisme plus tard
# si la progression doit être retrouvée sur plusieurs appareils.

PROGRESS_STORAGE_KEY = "frantsay_progress_v1"


def make_user_id(email: str) -> str:
    """Dérive un identifiant numérique stable à 6 chiffres à partir de l'e-mail
    (insensible à la casse et aux espaces superflus), sans base de données."""
    digest = hashlib.sha256(email.strip().lower().encode("utf-8")).hexdigest()
    return str(int(digest, 16) % 1_000_000).zfill(6)


def build_progress_sync_html(
    score: int,
    questions_done: int,
    level: str,
    email: str | None,
...
(function() {{
    try {{
        var win = window.parent || window;
        var STORAGE_KEY = "{PROGRESS_STORAGE_KEY}";
        var alreadyRestored = {"true" if restored else "false"};

        if (!alreadyRestored) {{
            // IMPORTANT : tant que la restauration n'a pas eu lieu pour cette
            // nouvelle session serveur (un F5 en crée toujours une), on ne
            // touche JAMAIS à localStorage en écriture ici. L'ancien code
            // sauvegardait d'abord l'état par défaut (score/niveau remis à
            // zéro par le F5) AVANT de relire la sauvegarde : il écrasait donc
            // systématiquement la progression existante avec des zéros avant
            // même de pouvoir la restaurer. On se contente ici de lire.
            // "__none__" signale explicitement au serveur "localStorage a été
            // consulté, rien à restaurer" : sans ce signal explicite, un
            // premier visiteur (rien en localStorage) ne fait jamais passer
            // progress_restored à True côté Python, et sa progression n'est
            // donc jamais sauvegardée non plus (branche "else" jamais atteinte).
            var toPush = win.localStorage.getItem(STORAGE_KEY) || "__none__";
            var doc = win.document;
            // Le widget caché (champ texte) peut ne pas encore être monté
            // dans le DOM au moment où ce composant s'exécute (F5 = tout
            // redémarre en parallèle) : on retente à intervalles courts
            // plutôt que d'abandonner silencieusement.
            var attemptsLeft = 20;
            (function tryPush() {{
                var target = doc.querySelector('.st-key-progress_bridge_slot input');
                if (target) {{
                    if (target.value !== toPush) {{
                        var setter = Object.getOwnPropertyDescriptor(win.HTMLInputElement.prototype, 'value').set;
                        setter.call(target, toPush);
            ...
            // Une fois la progression restaurée (ou confirmée absente) pour
            // cette session serveur, on peut sauvegarder l'état courant sans
            // risque d'écraser une sauvegarde qui n'a pas encore été relue.
...
with st.container(key="progress_bridge_slot"):
    progress_channel_value = st.text_input(
        "progress_channel",
        key="progress_channel_value",
        label_visibility="collapsed",
    )

if progress_channel_value and not st.session_state.progress_restored:
    # "__none__" est le signal explicite envoyé par le JS quand localStorage a
    # bien été consulté mais ne contenait aucune sauvegarde (première visite).
    # On distingue ce cas de la chaîne vide initiale (pas encore consultée)
    # pour ne jamais rester bloqué en "non restauré" : sans ce signal, un
    # nouvel utilisateur ne voyait jamais sa progression persister, faute de
    # passer un jour au rameau "else" (sauvegarde) du script JS.
    if progress_channel_value != "__none__":
        try:
            saved_progress = json.loads(progress_channel_value)
            st.session_state.score = int(saved_progress.get("score", st.session_state.score))
            st.session_state.questions_done = int(saved_progress.get("questions_done", st.session_state.questions_done))
            saved_level = saved_progress.get("level")
            if saved_level in LEVELS:
                st.session_state.level = saved_level
                st.session_state.model_sentence = random.choice(MODEL_SENTENCES[saved_level])

            saved_email = saved_progress.get("email")
            saved_pseudo = saved_progress.get("pseudo")
            saved_user_id = saved_progress.get("user_id")
            if saved_email and saved_pseudo and saved_user_id:
                st.session_state.user_email = saved_email
                st.session_state.user_pseudo = saved_pseudo
...
components.html(
    build_progress_sync_html(
        ...
        st.session_state.progress_restored,
    ),
    height=0,
)
...
# =============================================================================
# 5ter. IDENTIFICATION (e-mail + pseudo, sans mot de passe)
# =============================================================================
# Tant que l'utilisateur n'a pas été retrouvé dans le localStorage (ci-dessus)
# ni identifié via ce formulaire, on bloque l'accès au reste de l'application
# avec st.stop(). Une fois le formulaire validé, l'identité est stockée dans
# st.session_state puis synchronisée vers localStorage au prochain rerun par
# le pont défini en 5bis (même clé "frantsay_progress_v1"), ce qui permet de
# la retrouver automatiquement à chaque F5 ou retour sur le site.

# Validation stricte de l'adresse e-mail : l'ancienne règle ("@" présent et
# un "." après le dernier "@") laissait passer à peu près n'importe quoi tant
# que ça se terminait par "@gmail.com" (espaces, "@" en trop, partie locale
# vide...). Cette expression impose une partie locale et un domaine bien
# formés (pas d'espace, pas de points ou tirets en trop en début/fin, un
# domaine avec au moins une extension de 2 caractères ou plus).
...
if not st.session_state.identified:
    ...
    with st.container(key="auth_hero"):
        st.markdown(
            f"""<div class="auth-hero-row">{ROBOT_SVG}
<div><span class="eyebrow" style="color:rgba(255,255,255,.85)"><span class="tag tag-solid" style="background:rgba(255,255,255,.18);border-color:rgba(255,255,255,.35)">MG</span> TONGASOA.</span>
<h1 class="auth-hero-title">Bienvenue sur FRANTSAY</h1>
<p class="auth-hero-sub">Ta plateforme pour apprendre le français, étape par étape.</p></div></div>""",
            unsafe_allow_html=True,
        )

    st.markdown(
        '<div class="auth-features">'
        '<span class="tag">01 Grammaire</span>'
        '<span class="tag tag-green">02 Missions</span>'
        '<span class="tag tag-amber">03 Prononciation</span>'
        '<span class="tag tag-red">04 Quiz</span>'
        "</div>",
        unsafe_allow_html=True,
    )

    st.markdown('<div class="card auth-form-card">', unsafe_allow_html=True)
    st.markdown(
        '<span class="eyebrow">Créer mon profil</span>'
        '<p style="color:var(--muted);font-size:.8rem;margin:.3rem 0 .8rem 0">'
        "Indique ton e-mail et ton pseudo pour retrouver ta progression à chaque visite."
        "</p>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    st.markdown("### Mon identité")
    st.markdown(f"**ID :** #{safe_html(st.session_state.user_id)}")
    st.markdown(f"**Pseudo :** {safe_html(st.session_state.user_pseudo)}")
    if st.button("Changer de profil", key="logout_button", use_container_width=True):
...
with st.container(key="hero_box"):
    st.markdown(
        f"""<div class="eyebrow"><span class="tag tag-solid">MG</span> TONGASOA.</div>
...
<div class="hero-status-row">{status}{ROBOT_SVG}</div>""",
        unsafe_allow_html=True,
    )
