...
ROBOT_SVG = """<svg class="hero-robot-svg" viewBox="0 0 100 108" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Mascotte robot FRANTSAY" onclick='var el=this; el.classList.add("robot-clicked"); el.addEventListener("animationend", function h(e){ if(e.animationName==="robotClickBounce"){ el.classList.remove("robot-clicked"); el.removeEventListener("animationend", h); } });'>
<defs>
...
LIGHT_THEME = {
    ...
    "footer_bg": "rgba(255,255,255,.92)", "scheme": "light", "robot_eye": "#2563EB",
}
DARK_THEME = {
    ...
    "footer_bg": "rgba(30,41,59,.92)", "scheme": "dark", "robot_eye": "#38BDF8",
}
...
@media (hover: hover) and (pointer: fine) {
    div.stButton > button:hover { transform: scale(1.015); box-shadow: 0 6px 16px rgba(79, 70, 229, 0.38) !important; filter: brightness(1.05); }
}
div.stButton > button:active { transform: scale(0.97); }
div.stButton > button:focus-visible { outline: 2px solid var(--violet) !important; outline-offset: 2px; }
...
def build_progress_sync_html(score: int, questions_done: int, level: str, restored: bool) -> str:
    payload = json.dumps({"score": score, "questions_done": questions_done, "level": level})
    return f"""
<script>
(function() {{
    try {{
        ...
        // 1) On sauvegarde systématiquement l'état courant.
        win.localStorage.setItem(STORAGE_KEY, JSON.stringify({payload}));

        // 2) Tant que la progression n'a pas été restaurée pour cette session
        //    serveur, on tente de relire une sauvegarde existante et de la
        //    pousser vers Streamlit via le champ caché.
        if (!{"true" if restored else "false"}) {{
            var saved = win.localStorage.getItem(STORAGE_KEY);
            if (saved) {{
