...
- Audio recorder via audio_recorder_streamlit
...
/* Le fond Baobabs n'existe que sur l'ecran de bienvenue. */
body.frantsay-welcome .stApp {
    background:
        linear-gradient(180deg, rgba(13, 34, 28, .56), rgba(38, 22, 14, .68)),
        url("https://commons.wikimedia.org/wiki/Special:Redirect/file/The%20Avenue%20of%20the%20Baobabs%20in%20Madagascar%20near%20the%20city%20of%20Morondava%20at%20sunrise%20%2825%29.jpg")
        center / cover no-repeat fixed;
    background-color: #17352c;
}
body.frantsay-welcome .stApp::before,
body.frantsay-welcome .stApp::after {
    content: "";
    position: fixed;
    inset: 0;
    pointer-events: none;
...
body.frantsay-welcome .stApp::before { background: rgba(8, 24, 19, .16); }
body.frantsay-welcome .stApp::after {
    background: linear-gradient(180deg, rgba(8,20,18,.08), rgba(8,20,18,.22));
}
...
/* ===== REFONTE UI/UX V5 ===== */
:root {
    --native-bg: var(--background-color, #FBF8F3);
    --native-card: var(--secondary-background-color, #FFFFFF);
    --native-text: var(--text-color, #1A1410);
    --native-border: var(--border-color, #E7DED3);
}
.stApp { background: var(--native-bg) !important; color: var(--native-text) !important; }
body.frantsay-welcome .stApp, body.frantsay-welcome .stApp::before, body.frantsay-welcome .stApp::after { background: var(--native-bg) !important; }
body.frantsay-welcome .stApp::before, body.frantsay-welcome .stApp::after { display:none !important; }
.card,.lesson,.tip,.mini,.dashboard-stat,.identity-note,.auth-side-card,.ton-espace-nav,.st-key-hero_box {
    background:var(--native-card) !important; color:var(--native-text) !important; border-color:var(--native-border) !important;
}
.card p,.lesson p,.tip p,.mini p,.dashboard-stat small,.identity-note-text span { color:var(--muted) !important; }

.home-hero {
    position:relative; overflow:hidden; min-height:255px; margin-bottom:.8rem; padding:1.35rem;
    border-radius:24px; border:1px solid rgba(255,255,255,.18);
    background-image:linear-gradient(110deg,rgba(8,25,20,.88),rgba(8,25,20,.64) 58%,rgba(20,16,10,.55)),
    url("https://commons.wikimedia.org/wiki/Special:Redirect/file/The%20Avenue%20of%20the%20Baobabs%20in%20Madagascar%20near%20the%20city%20of%20Morondava%20at%20sunrise%20%2825%29.jpg");
    background-position:center; background-size:cover; box-shadow:0 18px 42px rgba(0,0,0,.18);
    display:flex; flex-direction:column; justify-content:space-between;
}
.home-hero-content { max-width:680px; position:relative; z-index:1; }
.home-hero .eyebrow { color:#F0B84A !important; }
.home-hero h1,.home-hero h2,.home-hero p { color:#fff !important; }
.home-hero h1 { margin:.45rem 0 .5rem; font-size:clamp(1.7rem,5vw,2.65rem); line-height:1.08; }
.home-hero p { margin:0; max-width:620px; color:rgba(255,255,255,.82) !important; line-height:1.65; }
.home-hero-status { margin-top:.9rem; }
.daily-tip { position:relative; z-index:1; margin:1.2rem -.2rem -.2rem; padding:.85rem 1rem; border-radius:14px; background:#E69A2A; color:#24180A; display:flex; align-items:center; gap:.7rem; box-shadow:0 8px 20px rgba(0,0,0,.16); }
.daily-tip .bulb { font-size:1.1rem; font-weight:900; }
.daily-tip b { color:#24180A !important; }
.daily-tip span { color:#3A260E !important; font-size:.78rem; line-height:1.45; }
.start-grid,.steps-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:.75rem; }
.start-card,.step-card { background:var(--native-card); color:var(--native-text); border:1px solid var(--native-border); border-radius:18px; padding:1rem; min-width:0; box-shadow:0 8px 24px rgba(var(--shadow-rgb),.07); }
.start-icon,.step-num { width:34px; height:34px; border-radius:10px; display:grid; place-items:center; background:var(--lamba-clair); color:var(--lamba-fonce); font-weight:900; margin-bottom:.65rem; }
.start-card h3,.step-card h3 { margin:0 0 .3rem; font-size:.92rem; color:var(--native-text) !important; }
.start-card p,.step-card p { margin:0; color:var(--muted) !important; font-size:.74rem; line-height:1.5; }
.section-title { margin:1.05rem 0 .6rem; font-size:.72rem; letter-spacing:.12em; text-transform:uppercase; font-weight:900; color:var(--muted); }

section[data-testid="stSidebar"] div[role="radiogroup"] > label { position:relative; padding-left:2.35rem !important; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label::before { position:absolute; left:.72rem; top:50%; transform:translateY(-50%); width:1.1rem; text-align:center; color:var(--sidebar-muted) !important; font-size:.9rem; font-weight:800; line-height:1; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(1)::before { content:"⌂"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(2)::before { content:"▦"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(3)::before { content:"▤"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(4)::before { content:"◇"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(5)::before { content:"◉"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(6)::before { content:"?"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:nth-of-type(7)::before { content:"○"; }
section[data-testid="stSidebar"] div[role="radiogroup"] > label:has(input:checked)::before { color:var(--sidebar-accent) !important; }
.st-key-sidebar_logout { border-top:1px solid var(--sidebar-border); margin-top:1rem; padding-top:1rem; }
.st-key-sidebar_logout div.stButton > button::before { content:"↪"; margin-right:.45rem; }
.sidebar-logo-sub { color:var(--sidebar-accent) !important; font-size:.5rem; letter-spacing:.13em; font-weight:800; margin-top:.18rem; }

.dashboard-stat.accent { background:linear-gradient(135deg,var(--tanety),var(--terre-foncee)) !important; color:#fff !important; }
.dashboard-stat.accent span,.dashboard-stat.accent b,.dashboard-stat.accent small { color:#fff !important; }
.audio-state { margin:.55rem 0 .8rem; padding:.65rem .8rem; border-radius:12px; background:var(--native-card); border:1px solid var(--native-border); color:var(--muted); font-size:.74rem; }
.audio-state b { color:var(--native-text) !important; }
@media (max-width:768px) {
    .start-grid,.steps-grid { grid-template-columns:1fr; }
    .home-hero { min-height:290px; padding:1rem; border-radius:20px; }
...
@media (max-width:480px) {
    ...
    .start-card,.step-card { padding:.9rem; }
}
...
def render_auth_screen() -> None:
    ...
    with col_left:
        ...
        auth_mode = st.selectbox("Acces", ["Connexion", "Creer un compte"], label_visibility="collapsed")
    ...
    with col_right:
        st.markdown(
            '<div class="auth-trust-line"><span>✓</span><b>Connexion securisee</b><small>Ton parcours reste prive.</small></div>',
            unsafe_allow_html=True,
        )
...
components.html(
    ...
    (function () {
        ...
        const page = %r;
        if (page === "Accueil") {
            body.classList.add("frantsay-welcome");
        } else {
            body.classList.remove("frantsay-welcome");
        }
    })();
    </script>""" % st.session_state.nav_page,
    height=0,
)
...
NAV_PAGES = ["Accueil", "Tableau de bord", "Grammaire", "Missions", "Prononciation", "Quiz", "Profil"]

with st.sidebar:
    st.markdown(
        '<div class="sidebar-logo"><div class="sidebar-logo-mark">F</div>'
        '<div><div class="sidebar-logo-word">FRANTSAY</div><div class="sidebar-logo-sub">L’ÂME DE MADAGASCAR</div></div></div>',
        unsafe_allow_html=True,
    )
...
if st.session_state.nav_page == "Accueil":
    ...
    st.markdown(
        f'''<section class="home-hero">
            <div class="home-hero-content">
                <span class="eyebrow">FRANTSAY · L’ÂME DE MADAGASCAR</span>
                <h1>Apprendre le français autrement.</h1>
                <p>Une expérience simple pour comprendre, pratiquer et progresser à ton rythme, avec des situations proches de la vie quotidienne à Madagascar.</p>
                <div class="home-hero-status">{status}</div>
            </div>
            <div class="daily-tip"><span class="bulb">*</span><div><b>Conseil du jour</b><br><span>Lis une phrase à voix haute, puis réutilise-la dans une situation réelle. La régularité compte plus que la quantité.</span></div></div>
        </section>''', unsafe_allow_html=True)

    st.markdown('<div class="section-title">Commencer</div>', unsafe_allow_html=True)
    st.markdown(
        '''<div class="start-grid">
            <div class="start-card"><div class="start-icon">B</div><h3>Grammaire</h3><p>Comprends les règles et corrige tes phrases avec des explications simples.</p></div>
            <div class="start-card"><div class="start-icon">M</div><h3>Missions</h3><p>Pratique le français dans des situations concrètes de la vie quotidienne.</p></div>
            <div class="start-card"><div class="start-icon">P</div><h3>Prononciation</h3><p>Lis des phrases à voix haute et reçois un retour automatique sur ta prononciation.</p></div>
        </div>''', unsafe_allow_html=True)

    st.markdown('<div class="section-title">Comment ça marche ?</div>', unsafe_allow_html=True)
    st.markdown(
        '''<div class="steps-grid">
            <div class="step-card"><div class="step-num">1</div><h3>Choisis</h3><p>Une leçon, une mission ou un exercice adapté à ton parcours.</p></div>
            <div class="step-card"><div class="step-num">2</div><h3>Pratique</h3><p>Écris, parle, écoute et réponds pour transformer la théorie en réflexes.</p></div>
            <div class="step-card"><div class="step-num">3</div><h3>Progresse</h3><p>Retrouve tes résultats et tes paliers dans ton tableau de bord privé.</p></div>
        </div>''', unsafe_allow_html=True)
    ...
    st.markdown('<div class="card"><span class="eyebrow"><span class="tag tag-amber">03</span>Prononciation interactive</span><h3 style="margin:.2rem 0;font-size:1rem">Entraîne ta prononciation</h3></div>', unsafe_allow_html=True)
    st.session_state.setdefault("recording_started", False)
    st.session_state.setdefault("recording_status", "Prêt à enregistrer")

    colA,colB=st.columns(2)
    with colA:
        if st.button("[R] Nouvelle phrase",key="new_sentence",use_container_width=True):
            st.session_state.model_sentence=random.choice(MODEL_SENTENCES[level])
            st.session_state.pronunciation_result=None
            st.session_state.last_audio_hash=None
    with colB:
        listen_model=st.button("[P] Écouter le modèle",key="listen_model",use_container_width=True)

    st.markdown('<div class="phrase-modele"><span class="eyebrow2">Phrase à lire à voix haute</span><h3>" '+safe_html(st.session_state.model_sentence)+' "</h3></div>',unsafe_allow_html=True)
    if listen_model:
        try: st.audio(make_audio(st.session_state.model_sentence),format="audio/mp3")
        except Exception as exc: st.error(f"Audio indisponible : {exc}")

    st.markdown('<div class="card"><span class="eyebrow">A toi de parler</span><h4 style="margin:.2rem 0;font-size:.95rem">Enregistre-toi</h4><p style="margin:0;font-size:.76rem;color:var(--muted)">Utilise Démarrer puis Arrêter. Dès que la capture audio se termine, l’analyse IA se lance automatiquement — aucun bouton Valider n’est nécessaire.</p></div>',unsafe_allow_html=True)
    ctl1,ctl2=st.columns(2)
    with ctl1:
        if st.button("[micro] Démarrer",key="start_recording",use_container_width=True):
            st.session_state.recording_started=True
            st.session_state.recording_status="Enregistrement en cours…"
    with ctl2:
        if st.button("[stop] Arrêter",key="stop_recording",use_container_width=True):
            st.session_state.recording_started=False
            st.session_state.recording_status="Arrêt demandé — analyse automatique dès réception de l’audio."
    st.markdown(f'<div class="audio-state"><b>État :</b> {safe_html(st.session_state.recording_status)}</div>',unsafe_allow_html=True)

    if AUDIO_RECORDER_AVAILABLE:
        audio_bytes=audio_recorder(text="Capturer la voix — relâcher pour arrêter",recording_color="#B83A24",neutral_color="#7A6E64",icon_name="microphone",icon_size="2x",key="audio_recorder_main")
        if audio_bytes is not None:
            audio_hash=hashlib.md5(audio_bytes).hexdigest()
            if audio_hash != st.session_state.last_audio_hash:
                st.session_state.recording_started=False
                st.session_state.recording_status="Capture terminée — analyse IA en cours…"
                if not api_available(): show_api_notice()
                else:
                    try:
                        with st.spinner("Analyse automatique de ta prononciation…"):
                            pronunciation_result=call_gemini_audio_structured(PRONONCIATION_PROMPT,audio_bytes,"audio/wav",f"Phrase modèle attendue : {st.session_state.model_sentence}",ReponsePrononciation)
                        ...
                        save_current_progress()
                        if pronunciation_result.get("score",0)>=80: st.balloons()
                    except Exception as exc:
                        st.session_state.recording_status="Analyse impossible"
                        st.error(f"Erreur lors de l'analyse vocale : {exc}")
            st.audio(audio_bytes,format="audio/wav")
    else:
        st.warning("audio_recorder_streamlit n'est pas installé. Installe-le avec : `pip install audio-recorder-streamlit`")
        st.info("En attendant, utilise l'écoute du modèle ci-dessus.")

    pronunciation_result=st.session_state.pronunciation_result
    if pronunciation_result:
        st.markdown('<div class="card"><h4 style="margin:.1rem 0 .5rem 0;font-size:.95rem">Feedback automatique</h4>',unsafe_allow_html=True)
        st.metric("Score de prononciation",f"{pronunciation_result.get('score',0)}/100")
        for point in pronunciation_result.get("points_forts",[]): st.markdown(f'<span class="tag tag-green">OK</span> {safe_html(point)}',unsafe_allow_html=True)
        fautes=pronunciation_result.get("fautes",[])
        if fautes:
            st.markdown("**Fautes précises détectées**")
            for f in fautes:
                st.markdown(f'<div class="faute"><span class="mot">{safe_html(f.get("mot",""))}</span> — entendu : " {safe_html(f.get("entendu",""))} " , attendu : " {safe_html(f.get("attendu",""))} "<br><span class="tag">i</span> {safe_html(f.get("conseil",""))}</div>',unsafe_allow_html=True)
        else: st.success("Aucune faute détectée — bravo !")
...
elif st.session_state.nav_page == "Profil":
    st.markdown(
        '<div class="card"><span class="eyebrow">Profil</span>'
        '<h3 style="margin:.2rem 0;font-size:1rem">Mon compte</h3>'
        '<p style="margin:0;font-size:.8rem;color:var(--muted)">Gere ici les informations de ton compte.</p></div>',
        unsafe_allow_html=True,
    )
    st.text_input("E-mail", value=st.session_state.user_email, disabled=True)
    st.text_input("Pseudo", value=st.session_state.user_pseudo or "Non renseigne", disabled=True)
    st.text_input("Niveau d'etudes", value=level, disabled=True)
