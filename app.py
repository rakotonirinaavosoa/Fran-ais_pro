...
/* === FOND PHOTOGRAPHIQUE BAOBABS AVEC OVERLAY SOMBRE (WCAG AAA) === */
.stApp {
    background: linear-gradient(
        rgba(26, 20, 16, 0.82),
        rgba(26, 20, 16, 0.88)
    ),
    url("https://images.unsplash.com/photo-1516026672322-bc52d61a55d5?w=1920&q=80") center/cover no-repeat fixed;
    color: var(--ink);
...
@media (prefers-color-scheme: dark) {
    .card, .lesson, .tip, .mini, .st-key-hero_box, .dashboard-stat, .hero-user-badge,
    .identity-note, .auth-side-card {
    ...
    .app-footer { background: rgba(26,20,16,.94); }
...
.st-key-hero_box {
    padding: 1rem 1.1rem;
    margin-bottom: .7rem;
    background: linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(245,237,227,0.95) 100%);
    overflow: hidden;
    position: relative;
}
...
def create_login_session(auth_user_id: str, refresh_token: str, session_id: str | None = None) -> str:
    """Create or replace the server-side session and persist only its opaque id client-side."""
    sid = session_id or (uuid.uuid4().hex + uuid.uuid4().hex)
    payload = {
        "session_id_hash": _hash_sid(sid),
        "auth_user_id": str(auth_user_id),
    ...
    db = get_db_client()
    existing = _extract_first_row(
        db.table("app_sessions").select("session_id_hash").eq("session_id_hash", _hash_sid(sid)).limit(1).execute()
    )
    if existing:
        db.table("app_sessions").update(payload).eq("session_id_hash", _hash_sid(sid)).execute()
    else:
        db.table("app_sessions").insert(payload).execute()

    # URL fallback is deliberately kept as a stable opaque session id. The refresh token
    # itself is never put in the browser URL. CookieManager is preferred when available.
    st.query_params["frantsay_sid"] = sid
    cm = get_cookie_manager()
    if cm is not None:
        try:
            cm.set(SESSION_COOKIE_NAME, sid, expires_at=datetime.now() + timedelta(days=30))
        except Exception:
            pass
    return sid
...
def restore_from_cookie() -> bool:
    ...
    try:
        ...
        create_login_session(str(row["auth_user_id"]), new_refresh, session_id=sid)
        return True
...
def call_gemini_structured(system_prompt: str, user_prompt: str, schema_class):
    schema_json = json.dumps(schema_class.model_json_schema(), sort_keys=True)
    raw = _call_gemini_structured_cached(system_prompt, user_prompt, schema_json)
...
def call_gemini_text(system_prompt: str, user_prompt: str) -> str:
    return _call_gemini_text_cached(system_prompt, user_prompt)


@st.cache_data(show_spinner=False, ttl=600, max_entries=64)
def _call_gemini_audio_cached(system_prompt: str, audio_bytes: bytes, mime_type: str, extra_text: str, schema_json: str) -> str:
    key = get_api_key()
    if not key:
        raise ValueError("Cle API Gemini manquante cote serveur (st.secrets).")
    client = genai.Client(api_key=key)
    response = client.models.generate_content(
        model=MODEL_NAME,
        contents=[types.Part.from_bytes(data=audio_bytes, mime_type=mime_type), extra_text],
        config=types.GenerateContentConfig(
            system_instruction=system_prompt,
            response_mime_type="application/json",
...
def call_gemini_audio_structured(system_prompt: str, audio_bytes: bytes, mime_type: str, extra_text: str, schema_class):
    schema_json = json.dumps(schema_class.model_json_schema(), sort_keys=True)
    raw = _call_gemini_audio_cached(system_prompt, audio_bytes, mime_type, extra_text, schema_json)
    return json.loads(raw)


@st.cache_data(show_spinner=False, ttl=1800, max_entries=128)
def make_audio_bytes(text: str, slow: bool = False) -> bytes:
    audio = io.BytesIO()
...
def make_audio(text: str, slow: bool = False) -> io.BytesIO:
    return io.BytesIO(make_audio_bytes(text, slow))
...
with st.sidebar:
    ...
    st.caption(f"Niveau : {safe_html(st.session_state.level)}")
    st.caption(f"Points : {st.session_state.score} · Activites : {st.session_state.questions_done}")
...
with st.container(key="hero_box"):
    st.markdown(
        f"""<div class="dashboard-topline"><div>
            <div class="eyebrow"><span class="tag tag-solid">FRANTSAY</span> ESPACE D'APPRENTISSAGE</div>
            <h1 class="hero-title">Ton espace d'apprentissage <span class="wave">*</span></h1>
            <p class="hero-sub">Pret a continuer ton apprentissage ? · Niveau {safe_html(level)}</p>
        </div></div>""",
        unsafe_allow_html=True,
    )

# =============================================================================
# BOUTON DASHBOARD avec expander pour les 4 metriques
# =============================================================================

if st.button("📊 Dashboard", key="dashboard_toggle_btn", use_container_width=False):
    st.session_state.show_dashboard = not st.session_state.show_dashboard
    st.rerun()

if st.session_state.show_dashboard:
    with st.expander("📊 Mes metriques", expanded=True):
        st.markdown(
...
# =============================================================================
# NAVIGATION "TON ESPACE" — Routeur central avec composants Streamlit natifs
# =============================================================================

st.markdown("---")

NAV_PAGES = ["Accueil", "Defis", "Grammaire", "Missions", "Prononciation", "Quiz"]

st.markdown('<div class="ton-espace-header"><div class="ton-espace-title">Ton Espace</div></div>', unsafe_allow_html=True)
...
if selected_page and selected_page != st.session_state.nav_page:
    st.session_state.nav_page = selected_page
    st.session_state.nav_nonce += 1
    st.rerun()

st.markdown('<div id="ton-espace-content"></div>', unsafe_allow_html=True)
st.markdown("---")

# =============================================================================
# 14. CONTENU DES PAGES (routees par nav_page)
# =============================================================================
...
if st.session_state.nav_page == "Accueil":
...
elif st.session_state.nav_page == "Defis":
    ...
    if AUDIO_RECORDER_AVAILABLE:
        audio_bytes = audio_recorder(
            ...
            recording_color="#B83A24",
            neutral_color="#7A6E64",
        ...
        st.warning("audio_recorder_streamlit n'est pas installe. Installe-le avec : `pip install audio-recorder-streamlit`")
        st.info("En attendant, tu peux utiliser la fonctionnalite d'ecoute du modele ci-dessus.")
