...
if st.session_state.nav_page == "Accueil":
    ...
    with st.expander("⚙️ Préférences", expanded=False):
        st.caption(f"Niveau d'études actuel : **{level}**")
        st.caption(f"Score cumulé : **{st.session_state.score}** — Exercices réalisés : **{st.session_state.questions_done}**")

    # --- 4. Confidentialité & droits ---
    with st.expander("📜 Confidentialité & droits", expanded=False):
        cgu_url = _secret("CGU_URL")
        privacy_url = _secret("PRIVACY_URL")
        if cgu_url:
            st.markdown(
                f'<a href="{html.escape(cgu_url)}" target="_blank" '
                'style="color:var(--tanety);font-weight:700;text-decoration:none;">'
                '📄 Conditions Générales d\'Utilisation →</a>',
                unsafe_allow_html=True,
            )
        else:
            st.caption("Conditions Générales d'Utilisation : document non encore configuré.")

        if privacy_url:
            st.markdown(
                f'<a href="{html.escape(privacy_url)}" target="_blank" '
                'style="color:var(--tanety);font-weight:700;text-decoration:none;">'
                '🔒 Politique de protection des données personnelles →</a>',
                unsafe_allow_html=True,
            )
        else:
            st.caption("Politique de protection des données personnelles : document non encore configuré.")
