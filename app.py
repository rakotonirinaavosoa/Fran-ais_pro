...
/* Étapes numérotées de l'atelier prononciation */
.step-head {
    display: flex; align-items: center; gap: .5rem; margin-bottom: .4rem;
}
.step-num {
    flex: none; width: 24px; height: 24px; border-radius: 50%;
    background: var(--purple); color: white; font-weight: 800; font-size: .78rem;
...
with tab_pron:
    ...
    st.markdown(
        '<div class="card"><div class="step-head"><span class="step-num">1</span>'
        '<span class="step-title">Enregistre ta voix</span></div>'
        '<p style="margin:0;font-size:.76rem;color:var(--muted)">'
        "Appuie sur Démarrer, lis la phrase à voix haute, puis Arrêter.</p></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="card"><div class="step-head"><span class="step-num">2</span>'
        '<span class="step-title">Importe ton enregistrement</span></div>'
        '<p style="margin:0;font-size:.76rem;color:var(--muted)">'
        "Appuie sur « 💾 Télécharger », puis choisis ce fichier ci-dessous.</p></div>",
        unsafe_allow_html=True,
    )
    ...
    # --- Étape 3 : Analyse — toujours visible, même sans fichier ---
    done_class = "step-done" if uploaded_audio is not None else ""
    st.markdown(
        f'<div class="card"><div class="step-head {done_class}"><span class="step-num">3</span>'
        '<span class="step-title">Analyse ma prononciation</span></div>',
        unsafe_allow_html=True,
    )

    if uploaded_audio is None:
        st.info("⬆️ Importe d'abord ton fichier audio à l'étape 2 pour débloquer l'analyse.")
    elif not api_available():
        show_api_notice()
    else:
        if st.button("✨ Analyser ma prononciation", key="analyze_pronunciation"):
            try:
