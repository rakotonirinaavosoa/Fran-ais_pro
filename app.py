...
def secrets_status() -> list[tuple[str, str, bool]]:
    """Renvoie [(NOM, libelle, present?), ...] sans jamais exposer les valeurs."""
    return [(name, label, bool(_secret(name))) for name, label in _REQUIRED_SECRETS]
...
def render_auth_screen() -> None:
    ...
    if not supabase_ready():
        st.markdown(
            ...
            <div class="config-alert">
                <div class="config-alert-icon">!</div>
                <div>
                    <div class="config-alert-title">Configuration Supabase incomplete</div>
                    <div class="config-alert-sub">
                        Une ou plusieurs cles attendues sont introuvables dans
                        <code>.streamlit/secrets.toml</code> (ou dans les Secrets Streamlit Cloud).
                    </div>
                </div>
            </div>
            """,
        ...
        with st.expander("Voir le detail (aucune valeur n'est jamais affichee)", expanded=True):
            for name, label, present in secrets_status():
                icon = "🟢" if present else "🔴"
                state = "detectee" if present else "manquante"
                st.markdown(f"{icon} **`{name}`** — {label} — *{state}*")
            st.caption(
                "Astuce : cette app accepte soit des cles a plat "
                "(`SUPABASE_URL = \"...\"`), soit une section imbriquee "
                "(`[supabase]` puis `url = \"...\"`), soit de vraies variables "
                "d'environnement systeme portant le meme nom."
            )
        st.stop()
    ...
    with col_right:
        st.markdown(
            ...
            '<div class="side-check"><span>v</span> Authentification Supabase</div>'
            '<div class="side-check"><span>v</span> Session restauree apres F5</div>'
            '<div class="side-check"><span>v</span> Niveau choisi une seule fois</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            '<div class="identity-note-icon">i</div>'
            '<div><b>Confidentialite</b><span>Tes donnees restent entre toi et toi. Aucun e-mail affiche publiquement.</span></div>'
            '</div>',
            unsafe_allow_html=True,
        )
