...
if st.session_state.nav_page == "Accueil":
    ...
    st.markdown(f'''<section class="home-hero"><div class="home-hero-content"><span class="eyebrow">FRANTSAY · L’ÂME DE MADAGASCAR</span><h1>Apprendre le français autrement.</h1><p>Une expérience simple pour comprendre, pratiquer et progresser à ton rythme, avec des situations proches de la vie quotidienne à Madagascar.</p><div class="home-hero-status">{status}</div></div><div class="daily-tip"><span class="bulb">💡</span><div><b>Conseil du jour</b><br><span>Lis une phrase à voix haute, puis réutilise-la dans une situation réelle. La régularité compte plus que la quantité.</span></div></div></section>''', unsafe_allow_html=True)
    ...
    if MIC_RECORDER_AVAILABLE:
        audio = mic_recorder(start_prompt="🎙️ Démarrer", stop_prompt="⏹️ Arrêter", just_once=True, use_container_width=True, format="wav", key="pronunciation_recorder")
