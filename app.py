...
with tab_pron:
    ...
    # --- Enregistreur automatique : dès l'arrêt, l'analyse se lance seule ---
    st.markdown(
        '<div class="card"><span class="eyebrow">🎙️ À toi de parler</span>'
        '<h4 style="margin:.2rem 0;font-size:.95rem">Enregistre-toi</h4>'
        '<p style="margin:0;font-size:.76rem;color:var(--muted)">'
        "Appuie sur le micro, lis la phrase à voix haute, puis appuie à nouveau pour arrêter. "
    ...
    if audio_bytes:
        ...
        if not api_available():
            show_api_notice()
        elif st.session_state.get("last_audio_hash") != audio_hash:
            try:
