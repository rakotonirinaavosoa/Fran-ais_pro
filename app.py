...
def create_login_session(auth_user_id: str, refresh_token: str) -> None:
    ...
    # API reelle de streamlit-cookies-controller 0.0.4 : set(nom, valeur)
    # uniquement. Pas de key=, path=, max_age=, secure=, same_site= --
    # ces kwargs n'existent pas sur cette librairie (c'etait le bug).
    # La duree de vie de 30 jours reste appliquee cote serveur (colonne
    # expires_at de app_sessions, verifiee dans restore_from_cookie),
    # meme si le cookie navigateur lui-meme n'a pas de Max-Age explicite.
    get_cookie_controller().set(SESSION_COOKIE_NAME, session_id)
...
def delete_login_session(session_id: str | None) -> None:
    ...
    try:
        ...
        try:
            # .delete() n'existe pas sur CookieController : la methode
            # s'appelle .remove(nom), sans argument key=.
            get_cookie_controller().remove(SESSION_COOKIE_NAME)
        except Exception:
            pass
...
def handle_email_confirmation_link() -> bool:
    """
    Traite le retour du lien de confirmation e-mail Supabase.

    Important : par defaut, Supabase renvoie les jetons dans le FRAGMENT
    de l'URL (#access_token=...), qui n'est jamais envoye au serveur --
    aucun backend (Streamlit inclus) ne peut le lire. C'est pourquoi le
    modele d'e-mail "Confirm signup" doit etre configure cote Supabase
    pour utiliser des parametres de QUERY STRING a la place :

        {{ .SiteURL }}?token_hash={{ .TokenHash }}&type=email

    st.query_params lit alors token_hash/type normalement, et on echange
    ce jeton contre une session via verify_otp() -- cote serveur, sans
    jamais exposer access_token/refresh_token dans l'URL visible.
    """
    ...
    try:
        ...
        # Lien expire (validite 24h), deja utilise, ou jeton invalide.
        st.session_state.email_confirm_error = (
        ...
        # On retire token_hash/type de l'URL dans tous les cas pour eviter
        # de retenter la verification (jeton a usage unique) a chaque rerun.
